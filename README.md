...
**Built by [Jay Guwalani](https://github.com/JayDS22)** for the [AI Chatbot to Guide User Workflow](https://www.jenkins.io/projects/gsoc/2026/project-ideas/ai-chatbot-to-guide-user-workflow/) GSoC 2026 project.

**[Live Demo](https://web-production-d484f.up.railway.app/)** | **[Demo Video](https://youtu.be/Wbwm0EWpu28)** | **[API Docs](https://web-production-d484f.up.railway.app/docs)**

---

### Watch the Demo

[![Watch Demo Video](docs/images/thumbnail.png)](https://youtu.be/Wbwm0EWpu28)
...
| "How do I set up X?" | Copy-paste from docs | Checks your installed plugins first, tells you what's missing |
...
| Intent routing | Single prompt | Multi-agent system with specialized agents |
| Without Jenkins | Doesn't work | Full demo mode with realistic mock build logs and errors |
...
**Render (free):** Push to GitHub, connect repo at render.com, set `GROQ_API_KEY`, deploy.
...
| "How do I set up a multibranch pipeline?" | WORKFLOW | Checks installed plugins, gives customized steps with Jenkinsfile |
...
Each agent receives live Jenkins state (or realistic mock data). The troubleshoot agent gets actual build logs with extracted error lines. The workflow agent knows which plugins you already have. The recommend agent checks compatibility before suggesting.

### RAG (Retrieval Augmented Generation)
20 curated Jenkins documentation chunks indexed in FAISS with sentence-transformer embeddings. Agents receive the top-3 most relevant docs alongside live context. Test retrieval directly at `/api/rag/search?q=your+query`.

### Demo Mode
When Jenkins isn't connected, the system uses realistic mock data: actual build log output, real error messages (npm not found, Docker build failures, test timeouts), and a realistic 42-plugin list. Reviewers can evaluate the full system without Docker.
...
Responses render as proper markdown with syntax-highlighted code blocks (Groovy, Bash, YAML). Jenkinsfile examples are actually readable.
...
|-------|-------------|--------|
| **Application** | This PoC: context-aware multi-agent + RAG + demo mode + cloud deploy | Done |
...
**Context-awareness + RAG**: The existing resources-ai-chatbot-plugin does RAG over docs. This PoC adds what's missing: live Jenkins state injection. The production version combines both.

**Multi-agent routing**: Different problems need different treatment. A build failure needs log analysis. A "how do I" question needs workflow guidance with plugin awareness. A plugin question needs compatibility checking. One prompt cannot do all three well. This mirrors the 5+ agent system I built at Aya Healthcare (2000+ concurrent users, P95 <180ms).

**Demo mode**: Reviewers must be able to run the code without Docker. The mock data is real Jenkins console output and real error messages. The agents produce compelling responses even on mock data.

**Single-port deployment**: Frontend served by FastAPI on the same port. One container, one URL, works on any cloud platform.
...
3. **Context-awareness works.** When it says "you already have the git plugin installed", it checked your actual instance.

4. **RAG integration.** FAISS vector store with sentence-transformer embeddings. Agents get relevant documentation alongside live context.

5. **Production-ready.** Works without Jenkins (demo mode), deploys to cloud, serves frontend on same port, proper error handling.
